...
export default async function TrainingPage({ searchParams }: { searchParams: Promise<{ mode?: string; session?: string; case?: string }> }) {
  await requirePageUser('student');
  const query = await searchParams;
  const mode: SessionMode = query.mode === 'osce' ? 'osce' : query.mode === 'practice' ? 'practice' : 'guided';
  return <TrainingWorkspace mode={mode} initialSessionId={query.session} initialCaseId={query.case} />;
}
