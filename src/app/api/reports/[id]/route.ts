...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return ok(await new AgentRepository(user).getReport(id));
  } catch (error) {
    const detail = errorFromUnknown(error);
    const status = detail.code === 'AUTH_REQUIRED' ? 401 : detail.code === 'FORBIDDEN' ? 403 : detail.code === 'REPORT_NOT_FOUND' ? 404 : 500;
